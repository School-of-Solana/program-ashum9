...
**Deployed Frontend URL (building):** https://synelar9.vercel.app
...
### Description
MetaID NFT is a unique digital identity system built on Solana that allows each wallet to mint exactly one unique NFT representing their identity on the blockchain. This dApp implements a robust identity verification system using Program Derived Addresses (PDAs) to enforce the one-NFT-per-wallet constraint, ensuring that each user's MetaID is truly unique and cannot be duplicated.
...
- **One NFT Per Wallet:** Enforced at the program level using PDA seeds, making it impossible for any wallet to mint multiple MetaID NFTs
...
- **Master Edition NFT:** Each MetaID is a true NFT with supply of 1, making it non-fungible and unique
...
- **Real-time Status:** Check if a wallet already has a MetaID before attempting to mint
...
2. **Check Existing MetaID**
   - Upon wallet connection, the dApp automatically checks if you already have a MetaID NFT
   - If found, your existing MetaID information will be displayed (owner, mint address, metadata URI, creation date)

3. **Mint Your MetaID NFT** (if you don't have one)
   - Enter your metadata URI (IPFS link to your profile JSON)
   - Click "Mint MetaID NFT" button
...
The MetaID NFT program is built using the Anchor framework and integrates with Metaplex's Token Metadata program for standardized NFT creation.
...
- **MetaID Account PDA:**
  ...
  - **Security:** This approach makes it cryptographically impossible to mint multiple MetaIDs for the same wallet, as the PDA address is deterministic and can only be initialized once.

The PDA derivation ensures:
1. **Uniqueness:** One MetaID per wallet (address collision is impossible)
2. **Discoverability:** Anyone can derive a wallet's MetaID PDA to check if it exists
...
- **mint_metaid(ctx: Context<MintMetaId>, metadata_uri: String)**
  - **Description:** Mints a unique MetaID NFT for the calling wallet
  - **Parameters:**
  ...
  - **Process:**
    ...
    2. Derives MetaID PDA from caller's wallet (fails if already exists)
    3. Initializes MetaID account with owner, URI, timestamp, and mint address
  ...
  - **Accounts Required:**
    ...
    - `metaid_account`: PDA to store identity data (initialized)
  ...
  - **Error Cases:**
    - PDA already exists (user has MetaID)
...
pub struct MetaIdData {
    ...
    /// The wallet address that owns this MetaID
    pub owner: Pubkey,
    ...
    /// Timestamp when MetaID was created (8 bytes)
    /// Unix timestamp for immutable creation record
    pub created_at: i64,
    
    /// NFT mint address (32 bytes)
    /// The SPL token mint for this MetaID NFT
    pub mint: Pubkey,
...
- **owner**: Essential for identifying MetaID ownership
...
- **Test 1: Successful MetaID Minting**
  - Creates a new wallet, airdrops SOL
  - Mints MetaID NFT with valid metadata URI
...
- **Test 2: Metadata Verification**
  - Fetches an existing MetaID account
...
- **Test 1: Duplicate Mint Prevention**
  - Attempts to mint a second MetaID for the same wallet
...
- **Test 3: Invalid Signer Detection**
  - User A attempts to mint MetaID for User B's wallet
